...
def cancel_upload():
    ...
    try:
        ...
        if content:
            ...
            twitter_data = platforms_posted.get('twitter', {})
            status = twitter_data.get('status')
            ...
            calendar_event_id = twitter_data.get('calendar_event_id')

            if calendar_event_id:
                try:
                    calendar_manager = ContentCalendarManager(user_id)
                    calendar_manager.delete_event(calendar_event_id)
                    current_app.logger.info(f"Deleted calendar event {calendar_event_id} for cancelled upload")
                except Exception as e:
                    current_app.logger.error(f"Error deleting calendar event: {e}")
