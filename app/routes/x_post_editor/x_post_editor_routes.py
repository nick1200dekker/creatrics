...
def check_ongoing_uploads():
    """Check if user has any uploads in 'uploading' status for X/Twitter"""
    try:
        user_id = get_workspace_user_id()
        ...
        try:
            ...
            for item in content_items:
                ...
                twitter_data = platforms_posted.get('twitter', {})

                if twitter_data.get('status') == 'uploading':
                    ongoing_uploads.append({
                        'content_id': item.get('id'),
                        'title': twitter_data.get('post_text', 'Untitled')[:50],
                        'created_at': item.get('created_at')
                    })
        except Exception as e:
            current_app.logger.error(f"Error checking ongoing uploads: {e}", exc_info=True)
...
def cancel_upload():
    ...
    try:
        ...
        if content:
            ...
            for event in events:
                post_text = twitter_data.get('post_text', '')
                if event.get('content_id') == content_id or event.get('title') == post_text[:50]:
                    calendar_manager.delete_event(event.get('id'))
                    current_app.logger.info(f"Deleted calendar event {event.get('id')} for cancelled upload")
                    break

            current_app.logger.info(f"Cancelled upload {content_id} for user {user_id}")
            return jsonify({'success': True})
        else:
            return jsonify({'success': False, 'error': 'Content not found'}), 404

    except Exception as e:
        current_app.logger.error(f"Error cancelling upload: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500