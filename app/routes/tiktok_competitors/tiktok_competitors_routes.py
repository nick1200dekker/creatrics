...
# REMOVED: Avatar downloading/storage functions
# Storing avatars = copyright infringement
# We no longer download or host TikTok profile images
            ...
            for entry in videos_data:
                try:
                    ...
                    channels_dict[sec_uid] = {
                        ...
                        # Avatar URL only (not downloaded/stored)
...
def add_competitor():
    ...
    try:
        ...
        # Get username to check blocklist
        check_username = None
        if account_data_from_search:
            check_username = account_data_from_search.get('username', '').lower()
        elif account_url:
            tiktok_api = TikTokAPI()
            check_username = tiktok_api.extract_username(account_url)
            if check_username:
                check_username = check_username.lower()

        # Check if user has opted out
        if check_username:
            blocklist_ref = db.collection('tiktok_optout_blocklist').document(check_username)
            if blocklist_ref.get().exists:
                return jsonify({
                    'success': False,
                    'error': 'This TikTok creator has opted out of data collection. We cannot track this account.'
                }), 403
