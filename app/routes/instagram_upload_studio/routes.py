...
@bp.route('/api/init-instagram-upload', methods=['POST'])
@auth_required
def init_instagram_upload():
    """Initialize Instagram upload - generates signed URL for direct browser-to-Firebase upload"""
    try:
        user_id = g.user.get('id')
        ...
        try:
            ...
            logger.info(f"Generated signed upload URL for Instagram direct browser upload: {file_path}")
        except Exception as e:
            logger.error(f"Error generating signed upload URL for Instagram: {e}")
        ...
        logger.error(f"Error initializing Instagram upload: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500


@bp.route('/api/confirm-instagram-upload', methods=['POST'])
@auth_required
def confirm_instagram_upload():
    """Confirm Instagram upload completed - makes blob public"""
    try:
        user_id = g.user.get('id')
        data = request.get_json()
        ...
        logger.info(f"Made Instagram upload public: {public_url}")
        ...
        logger.error(f"Error confirming Instagram upload: {e}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_upload_media():
    """DEPRECATED: Use init-instagram-upload + confirm-instagram-upload for large files
